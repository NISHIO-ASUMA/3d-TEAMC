...
EXPLOSION g_Explosion[MAX_EXPLOSION];				// 構造体変数
...
void InitExplosion(void)
{
	...
	{
		g_Explosion[nCnt].pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);	// 座標
		g_Explosion[nCnt].move = D3DXVECTOR3(0.0f, 0.0f, 0.0f);	// 移動量
		g_Explosion[nCnt].nLife = 0;							// 寿命
		g_Explosion[nCnt].nCounterAnim = 0;						// アニメーションカウント
		g_Explosion[nCnt].fHeight = 0;							// 高さ
		g_Explosion[nCnt].fWidth = 0;							// 横幅
		g_Explosion[nCnt].nPatternAnim = 0;						// パターンカウント
		g_Explosion[nCnt].speed = 10.0f;						// 速度
		g_Explosion[nCnt].radius = 30.0f;						// 半径
		g_Explosion[nCnt].nType = EXPLOSION_MOVE;				// 種類
...
void SetExplosion(D3DXVECTOR3 pos,D3DXCOLOR col, int nLife,float fWidth,float fHeight, int nType)
{
	...
	for (int nCnt = 0; nCnt < MAX_EXPLOSION; nCnt++)
	{
		if (g_Explosion[nCnt].bUse == false)
		{
			...
			g_Explosion[nCnt].fWidth = fWidth;	// 横幅
			g_Explosion[nCnt].fHeight = fHeight;// 高さ
			g_Explosion[nCnt].col = col;		// カラー
			g_Explosion[nCnt].nLife = nLife;	// 寿命
			g_Explosion[nCnt].nType = nType;	// 種類
